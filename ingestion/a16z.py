"""a16z Future blog RSS feed ingestion."""
...
async def ingest(conn: Connection):
    """Fetch and parse a16z blog RSS feed."""
    logger.info("Ingesting a16z RSS feed")
    try:
        async with httpx.AsyncClient(timeout=30, follow_redirects=True) as client:
            resp = await client.get(FEED_URL, headers={"User-Agent": "VCPulse/1.0"})
            resp.raise_for_status()
    except Exception as e:
        logger.error(f"Failed to fetch a16z feed: {e}")
        return
    ...
    for entry in feed.entries:
        ...
        conn.execute(
            """INSERT OR IGNORE INTO articles
               ...
               VALUES (?, ?, ?, ?, ?, ?)""",
            (title, summary[:500] if summary else None, "a16z", link, tags, pub_dt),
        )
        count += 1

    conn.commit()
    logger.info(f"a16z: ingested {count} articles")
